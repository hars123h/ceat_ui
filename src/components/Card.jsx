...
const Card = ({ pre_sale, long_plan_state, product_type, product_image, plan_name, plan_type, plan_amount, plan_daily_earning, plan_cycle, handleClick }) => {
  ...
  return (
    <div className='mx-2  bg-selected shadow-ceatShadow3 pb-1'  >
      {/* <div className="title text-[#464945] font-bold text-lg">{plan_name}</div>         */}
      <div className="info text-xs flex flex-col items-center">
        <img src={product_image} alt="comp_img" className=' h-[150px] w-full mb-1' />
        ...
        <div className=' w-full grid grid-cols-2 p-1 mt-2 gap-2 text-[13px]'>
          <div className="basic_info  text-white flex justify-start gap-1 bg-[#718fe9]  py-3 px-2 mx-1">
            <div className='font-bold'>Price: </div>
            <div className=' text-white font-bold'>&#8377;{new Intl.NumberFormat().format(plan_amount)}</div>
          </div>
          <div className="basic_info text-white flex justify-start gap-1 bg-[#718fe9] py-3  mx-1">
...

      {
        pre_sale === false ? (
          product_type === 'long' ? (
            <div className="cursor-pointer btn text-white font-semibold text-center  py-2  px-1 mt-5 text-md mb-2 shadow-xs  w-[55%] mx-auto bg-selected border-white border"
